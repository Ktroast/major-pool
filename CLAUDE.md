...
- **Apple Mail consumes magic links before the user clicks them.** macOS Mail's "Protect Mail Activity" (on by default) pre-fetches every URL in incoming mail in the background. By the time the user clicks the Supabase confirmation link, the one-time token has been spent and the redirect lands on `/?#error=access_denied&error_code=otp_expired`. Workaround for users: copy-paste the link into the address bar, or turn off Mail → Settings → Privacy → "Protect Mail Activity". The same family of issue affects HEY and iCloud's `www-mail.icloud-sandbox.com` wrapper. The durable fix is switching from clickable-link flow to a typed OTP code (`signInWithOtp` + `verifyOtp`) — see PLAN_AUTH.md "Auth UX follow-ups".
- **Email collision during claim falls through to OTP silently.** When `updateUser({ email })` hits an email already attached to another account, the error is caught and `sendSignInLink` retries via `signInWithOtp`. The user sees no UI difference, but the magic link they then click signs them into the *other* account — so any pool memberships on their original anonymous user appear to vanish. This is the explicit phase 3 design choice (one intent, no branching), but it's confusing during testing or in the wild when a user has already claimed the same email elsewhere. Recovery is manual SQL reparenting of `user_pools` rows from the orphaned anonymous `user_id` to the claimed `user_id` — write a runbook (PLAN_AUTH.md "Auth UX follow-ups") before this happens at scale.
